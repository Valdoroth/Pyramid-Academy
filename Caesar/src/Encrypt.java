...
public class Encrypt {
    ...
    public static String encryptMessage(String message){
        ...
        if(1+keyCode > 70) slide = (1+keyCode-70)%70 ;
        else slide = (1+keyCode)%70;

        // This creates the array of letters shifted.
        for(int i = 0; i<70; i++) {
            if(i + slide > 70) code[i] = codeBase[i+slide-(71)];
            else code[i] = codeBase[i+slide-1];
        }

        for(char i: codeBase) letterList.add(i);

        // This encrypts the message and returns that string value of.
